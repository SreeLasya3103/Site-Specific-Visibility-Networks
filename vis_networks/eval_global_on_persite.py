...
def compute_metrics(pred_classes, true_classes, class_values):
    ...
    if ss_tot.item() == 0.0:
        r2 = float("nan")          # undefined, not perfect
    else:
...
def main():
    ...
    if report.missing_keys:
        raise SystemExit(f"ERROR: {args.checkpoint} is missing "
                         f"{len(report.missing_keys)} tensors; those layers would "
                         f"stay randomly initialised:\n  {report.missing_keys}")
    model(probe.unsqueeze(0))                 # initialise LazyLinear layers               # initialise LazyLinear layers
