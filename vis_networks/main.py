...
def prepare_dataloaders(config, split_sets):
    ...
    train_loader = DataLoader(split_sets[0], cf.batch_size, shuffle=True, num_workers=num_workers, pin_memory=True, drop_last=True)
    no_aug_train_loader = DataLoader(split_sets[1], cf.batch_size, shuffle=False, num_workers=num_workers)
    val_loader = DataLoader(split_sets[2], cf.batch_size, shuffle=False, num_workers=num_workers, pin_memory=True, drop_last=False)
    test_loader = DataLoader(split_sets[3], cf.batch_size, shuffle=False, num_workers=num_workers, pin_memory=True, drop_last=False)
...
def prepare_model(config, loader):
    ...
    #If an existing model is specified, create a new one and load the existing one's weights
        ...
        report = model.load_state_dict(state_dict, False)
        if report.missing_keys:
            raise SystemExit(f"ERROR: {cf.existing_model} is missing "
                             f"{len(report.missing_keys)} tensors:\n  {report.missing_keys}")
