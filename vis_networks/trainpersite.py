...
def extract_site_id(path: str) -> str:
    """Extract site ID from sample path."""
...
def split_site_data(samples: list, train_ratio: float = 0.8, seed: int = 42,
    ...
    # Group the frames captured together, then stratify those groups by class.
...
def load_pretrained_model(cf, model_path, sample_tensor):
    ...
    # strict=False plus LazyLinear silently leaves unmatched layers random.
    if report.missing_keys:
        raise SystemExit(f"ERROR: {model_path} is missing {len(report.missing_keys)} "
                         f"tensors; those layers would stay randomly initialised:\n"
                         f"  {report.missing_keys}")
    extra = [k for k in report.unexpected_keys
             if k not in ("example_input", "example_output")]
    if extra:
        print(f"  NOTE: checkpoint carries {len(extra)} unused tensors: {extra}")
...
def evaluate(model, loader, loss_fn, device, use_amp):
    model.eval()
    all_outputs, all_targets, all_paths = [], [], []
...
def train_one_site(
    ...
    model.to(device)
    # Same optimiser as pooled training: plain Adam, no weight decay, no schedule.
    ...
    patience_counter = 0
    history = []
    best_outputs = None   # test logits at the epoch chosen on validation
    best_targets = None   # matching one-hot targets
    best_paths = None     # the paths those rows actually came from
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss = evaluate(model, sel_loader, loss_fn, device, use_amp)[0]

        record = {"train_loss": train_loss, "val_loss": val_loss, "epoch": epoch}
        ...
        if val_loss < best_val_loss:
            ...
            # The test set is read only to report the epoch already chosen.
            test_loss, outputs, targets, paths = evaluate(
                model, test_loader, loss_fn, device, use_amp)
            met = compute_metrics(outputs, targets, class_names)
            met["train_loss"] = train_loss
            met["val_loss"] = val_loss
            met["test_loss"] = test_loss
            met["epoch"] = epoch
            best_test_metrics = met
            best_outputs = outputs.clone()
            best_targets = targets.clone()
    ...
    return (best_test_metrics, best_epoch, history,
            best_outputs, best_targets, best_paths)
...
def main():
    ...
    parser.add_argument("--val_ratio", type=float, default=0.2,
                        help="Fraction of each site's training portion held out for epoch selection")
    ...
    print(f"Val ratio:       {args.val_ratio} (of the training portion)")
    ...
    for i, site_id in enumerate(sorted_sites, 1):
        ...
        train_samples, val_samples, test_samples = split_site_data(
        ...
        # Normalisation statistics come from un-augmented images, as in main.py.
        stats_loader = make_loader(
            cf, train_samples, transformer, batch_size=min(16, n_train), shuffle=False
        )
        val_loader = None
        if n_val >= 1:
            val_loader = make_loader(
                cf, val_samples, transformer, batch_size=min(16, n_val), shuffle=False
            )
        ...
        if global_model is not None:
            ...
            model = create_fresh_model(cf, sample_tensor, train_loader=stats_loader, seed=args.seed + i)


        # Train
        best_met, best_ep, history, site_outputs, site_targets, site_paths = train_one_site(
            model=model,
        ...
        global_pred_classes.append(torch.argmax(sm, dim=1))           
        global_true_classes.append(torch.argmax(site_targets, dim=1))
    ...
    all_pred = torch.cat(global_pred_classes, dim=0)
    all_true = torch.cat(global_true_classes, dim=0)
